				...
				void CCreator :: Load ( ) {
					// Get Path
						...
						if ( FileExists ( tCreatorProductionSourceCodeFilePath ) == FALSE ) {
							MessageBox ( 0 , "Creator Production Source Code File not found. Please adapt the registry setting HKEY_LOCAL_MACHINE\\" + RegistryLocation ( ) + "\\CreatorProductionSourceCodeFilePath" + " and restart the program.", ApplicationName ( ) , 0 ) ;
							exit ( 0x0 ) ;
						}
					// Load Text File
						CString* tString ;
