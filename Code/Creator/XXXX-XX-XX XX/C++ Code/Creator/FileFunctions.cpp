...
// Methods
	...
	void SaveString ( CString* aText , CString aFilePath ) {
		// Delete existing file
			if ( FileExists ( aFilePath ) ) {
				...
				Sleep ( 30 ) ; // Stupid Quick & Dirty solution to DeleteFile leaving the file in use for a while. Or something.
			}
		// Declare File Handle
			HANDLE tFileHandle ;
		// Open File
	...
	CString ResolveInvalidFileNameCharacters ( CString aValue ) {
		aValue . Replace ( "\\" , "Back Slash" ) ;
		aValue . Replace ( "/" , "Foreward Slash" ) ;
		aValue . Replace ( "?" , "Question Mark" ) ;
		aValue . Replace ( "*" , "Asterisk" ) ;
		aValue . Replace ( ":" , "Colon" ) ;
		aValue . Replace ( "\"" , "Double Quotes" ) ;
		aValue . Replace ( "<" , "Smaller Than" ) ;
		aValue . Replace ( ">" , "Greater Than" ) ;
		aValue . Replace ( "|" , "Pipeline" ) ;
		return aValue ;
	} ;
