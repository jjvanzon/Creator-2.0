...
// Microsoft Generated Code
	...
	DDX_Control(pDX, IDC_EXECUTEBUTTON, ExecuteButton);
	...
	BEGIN_MESSAGE_MAP(TreeControl, CFormView)
		//{{AFX_MSG_MAP(TreeControl)
			...
			ON_WM_SIZE()
	ON_BN_CLICKED(IDC_EXECUTEBUTTON, OnExecuteButtonClicked)
	//}}AFX_MSG_MAP
	END_MESSAGE_MAP()
	...
	// Document
			...
			TreeControlDoc :: TreeControlDoc ( ) {
				...
				// Add Root Item
					...
					// Set Example Data Root
						...
						// Quick & dirty hard coded picking of program
							// Use a custom structure from a file
								DataStructureCreator = new CCreator ; // Not sure you need a whole new Creator, though.
							// Set source code path
								CString tFilePath ;
								tFilePath = "E:\\Bulk\\Jan-Joost van Zon\\Software Development\\1. Products\\2. Code\\1. Software System\\XXXX-XX-XX XX\\7. Creator 2.0\\Sound\\XXXX-XX-XX XX\\J Sound.cre" ;
								// tFilePath = "..\\..\\..\\..\\..\\Sound\\XXXX-XX-XX XX\\J Sound.cre" ;
								// tFilePath = "..\\..\\..\\..\\..\\Math\\Objects\\XXXX-XX-XX XX\\J Math  .  Objects.cre" ;
								// tFilePath = "..\\..\\..\\..\\..\\Controls\\XXXX-XX-XX XX\\Code\\J Controls.cre" ;
								//tFilePath = "E:\\Bulk\\Jan-Joost van Zon\\Software Development\\1. Products\\2. Code\\1. Software System\\XXXX-XX-XX XX\\7. Creator 2.0\\Code\\Creator\\XXXX-XX-XX XX\\Creator Code\\Creator.cre" ;
							// Load file as structure
								DataStructure = DataStructureCreator -> LoadStructure ( tFilePath ) ;
							// Set first class of structure as the root of the tree
								DataRoot -> GenericClass ( DataStructure -> RelatedLists ( ) -> Item ( "Classes" ) -> Item ( 0 ) ) ;
								//DataRoot -> GenericClass ( DataStructure -> RelatedLists ( ) -> Item ( "Classes" ) -> Item ( 5 ) ) ;
		...
		// Layer 2 : Main Operations
			...
			// Methods
				...
				void TreeControlDoc :: Execute ( long aPosition ) {
					// Conditions
						if ( ! IndexValid ( aPosition , TreeItemArray . GetSize ( ) ) ) { return ; }
					// Switch by Tree Item Type
						CString tItemType = GetAttribute ( aPosition , "Item Type" ) ;
						if ( tItemType == MethodItemType ( ) ) {
							CItemMethod* tItemMethod = ItemMethodArray . GetAt ( aPosition ) ;
							tItemMethod -> Execute ( ) ;
						}
					// Refresh
						Refresh ( 0 ) ;
			...
			// Properties
				...
				CString TreeControlDoc :: ItemType ( long aPosition ) {
					// Conditions
						// if ( ! IndexValid ( aPosition , TreeItemArray . GetSize ( ) ) ) { return "" ; }
					// Return Value
						return GetAttribute ( aPosition , "Item Type" ) ;
				} ;

				BOOL TreeControlDoc :: IsNothing ( long aPosition ) {
					if ( ItemType ( aPosition ) == RootItemType ( )  ) {
						if ( DataRoot == 0 ) { return TRUE ; } else { return FALSE ; } ;
					}
					if ( ItemType ( aPosition ) == RelatedItemItemType ( )  ) {
						if ( RelatedItemArray . GetAt ( aPosition ) -> Item ( ) == 0 ) { return TRUE ; } else { return FALSE ; } ;
					}
					if ( ItemType ( aPosition ) == RelatedListItemItemType ( ) ) {
						CRelatedList* tRelatedListItemRelatedList = RelatedListItemRelatedListArray . GetAt ( aPosition );
						long tRelatedListItemPosition = RelatedListItemPositionArray . GetAt ( aPosition ) ;
						if ( tRelatedListItemRelatedList -> Item ( tRelatedListItemPosition ) == 0 ) { return TRUE ; } else { return FALSE ; } ;
					}
					return TRUE ;
				} ;
		...
		// Layer 3 : Branch Offs of Main Operations
			...
			// Expand
				...
				// ( Layer 3 . 1 . 3 )
					...
					long TreeControlDoc :: ExpandItemMethods ( long aPosition , long aLevel , CItem* aDataItem , CItem* aTreeItem ) {
						CItem* tChild ;
						CItemMethods* tItemMethods = aDataItem -> ItemMethods ( ) ;
						// Title
							if ( tItemMethods -> ListCount ( ) > 0 ) {
								tChild = aTreeItem -> RelatedLists ( ) -> Item ( "Children" ) -> Add ( ) ;
								tChild -> RelatedItems ( ) -> Item ( "Parent" ) -> Item ( aTreeItem ) ; // Manually Synchronize Relation
								Insert (
									// a Position
						...
						long i ;
						CString tString ;
						for ( i = 0 ; i <= tItemMethods -> StorageCount ( ) - 1 ; i ++ ) {
							if ( tItemMethods -> PositionUsed ( i ) == TRUE ) {
								CItemMethod* tItemMethod = tItemMethods -> Item ( i ) ;
								tString = tItemMethod -> Name ( ) ;
								tChild = aTreeItem -> RelatedLists ( ) -> Item ( "Children" ) -> Add ( ) ;
								tChild -> RelatedItems ( ) -> Item ( "Parent" ) -> Item ( aTreeItem ) ; // Manually Synchronize Relation
								Insert (
									// a Position
			...
			// Refresh
				...
				// ( Layer 3 . 3 . 3 )
					...
					void TreeControlDoc :: RefreshItemMethods ( long* aPosition , long aLevel , CItem* aDataItem , CItem* aTreeItem , long* aChildIndex  ) {
						// Declarations
							CItem* tChild ;
							CItemMethods* tItemMethods = aDataItem -> ItemMethods ( ) ;
							CRelatedList* tTreeChildren = aTreeItem -> RelatedLists ( ) -> Item ( "Children" ) ;
						// Title
							if ( tItemMethods -> ListCount ( ) > 0 ) {
								*aChildIndex += 1 ;
									if ( tTreeChildren -> PositionUsed ( *aChildIndex ) == FALSE ) {
										tTreeChildren -> Add ( ) ; // Really assumptious, about there's only 1 child too little.
									}
								tChild = tTreeChildren -> Item ( *aChildIndex ) ;
								*aPosition += 1 ;
								Insert (
		...
		// Layer 5 : Insert & Delete
			...
			void TreeControlDoc :: Insert (
				...
				// Add Position
					...
					ItemMethodArray . SetSize ( ItemMethodArray . GetSize ( ) + 1 ) ;
				// Shift all down
					long i ;
					for ( i = TreeItemArray . GetSize ( ) - 1 ; i >= aPosition + 1 ; i -- ) {
						...
						ItemMethodArray . SetAt ( i , ItemMethodArray . GetAt ( i - 1 ) ) ;
				...
				// Fill in item
					...
					if ( aItemType == MethodItemType ( ) ) {
						ItemMethodArray . SetAt ( aPosition , ( CItemMethod* ) aDataItem ) ;
					}
			} ;
		...
		// Event Procedures ( / Message Mappings )
			...
			void TreeControl :: OnAddButtonClicked ( ) 
			{
				// Conditions
					// Item Type Correct
						if (
							Document ( ) -> ItemType ( CurrentLine ( ) ) != Document ( ) -> RelatedListItemItemType ( ) &&
							Document ( ) -> ItemType ( CurrentLine ( ) ) != Document ( ) -> RelatedListItemType ( )
						) {
							MessageBox ( "You are not positioned on a list.", ApplicationName ( ) , MB_OK ) ;
							return ;
						}
				// Functionality
			...
			void TreeControl :: OnRemoveButtonClicked ( ) 
			{
				// Conditions
					// Item Type Correct
						if ( Document ( ) -> ItemType ( CurrentLine ( ) ) != Document ( ) -> RelatedListItemItemType ( ) ) {
							MessageBox ( "Current item is not in a list.", ApplicationName ( ) , MB_OK ) ;
							return ;
						}
					// User Confirmation
						if ( MessageBox ( "Are you sure you wish to remove the selected item?", ApplicationName ( ) , MB_YESNO ) == IDNO ) {
							return ;
						}
				// Functionality
			...
			void TreeControl :: OnCreateButtonClicked ( ) 
			{
				// Conditions
					// Item Type Correct
						if (
							Document ( ) -> ItemType ( CurrentLine ( ) ) != Document ( ) -> RootItemType ( ) &&
							Document ( ) -> ItemType ( CurrentLine ( ) ) != Document ( ) -> RelatedItemItemType ( ) &&
							Document ( ) -> ItemType ( CurrentLine ( ) ) != Document ( ) -> RelatedListItemItemType ( ) 
						) {
							MessageBox ( "Current item is not an object.", ApplicationName ( ) , MB_OK ) ;
							return ;
						}
					// User Confirmation						
						if ( Document ( ) -> IsNothing ( CurrentLine ( ) ) == FALSE ) {
							if ( MessageBox ( "Are you sure you wish to overwrite the selected object reference with a new object?", ApplicationName ( ) , MB_YESNO ) == IDNO ) {
								return ;
							}
						}
				// Functionality
			...
			void TreeControl :: OnSetToNothingButtonClicked ( )
			{
				// Conditions
					...
					// Not Already Nothing
						if ( Document ( ) -> IsNothing ( CurrentLine ( ) ) == TRUE ) {
							MessageBox ( "Current item is already nothing.", ApplicationName ( ) , MB_OK ) ;
							return ;
						}
					// User Confirmation
						if ( MessageBox ( "Are you sure you wish to set the selected item to nothing?", ApplicationName ( ) , MB_YESNO ) == IDNO ) {
							return ;
						}
				// Functionality
			...
			void TreeControl :: OnAssignItemButtonClicked ( ) 
			{
				// Conditions
					...
					// User Confirmation
						if ( Document ( ) -> IsNothing ( CurrentLine ( ) ) == FALSE ) {
							if ( MessageBox ( "Are you sure you wish to overwrite the selected object reference?", ApplicationName ( ) , MB_YESNO ) == IDNO ) {
								return ;
							}
						}
				// Functionality
			...
			void TreeControl::OnExecuteButtonClicked() 
			{
				// Conditions
					// Item Type Correct
						if ( Document ( ) -> ItemType ( CurrentLine ( ) ) != Document ( ) -> MethodItemType ( ) ) {
							MessageBox ( "Current item is not a method.", ApplicationName ( ) , MB_OK ) ;
							return ;
						}
					// User Confirmation
						if ( MessageBox ( "Are you sure you wish to execute this method?" , ApplicationName ( ) , MB_YESNO ) == IDNO ) {
							return ;
						}
				// Functionality
					Document ( ) -> Execute ( CurrentLine ( ) ) ;
					Fill ( ) ;
			}
		...
		// Base Class Event Procedures ( / Overrides )
			...
			void TreeControl :: OnInitialUpdate()  {
				...
				// Quick Fix : To Fill OnLoad I use OnInitialUpdate, because I can't find anything that better represents OnLoad.
					...
					ValueTextBox . SetLimitText ( 4294967295 ) ;
					Fill ( ) ;
			} ;
