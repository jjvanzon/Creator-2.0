				...
				// Constructor & Destructor
					...
					CItemAttribute :: CItemAttribute ( ) {
						...
						// Test Loading a Library with Custom Code
							...
							tModuleHandle = LoadLibrary ( "E:\\Bulk\\Jan-Joost van Zon\\Computer Programming\\1. Systems\\2. Code\\1. Software System\\XXXX-XX-XX XX\\7. Creator 2.0\\Code\\Creator\\XXXX-XX-XX XX\\C++ Code\\Example Method Library\\Debug\\ExampleMethodLibrary.dll" ) ;
								...
								if ( MethodLibraryHandle != 0 ) {
									...
									// Call DLL Function
										try {
											...
											static bool tShowErrors = true ;
											if ( tShowErrors ) {
												if ( MessageBox ( 0 , "Runtime error in " + Name ( ) , "" , MB_OKCANCEL ) ) {
													tShowErrors = false ;
												}
											}
										}
							...
							// Load Method Library
								if ( sGenericClassAttribute != 0 ) {
									// Compose path from module path, module name, class name and attribute name
										...
										// Compose DLL path
											CString tLibraryPath = tModuleFolder + "\\" + ResolveInvalidFileNameCharacters ( tModuleName + "  .  " + tClassName + "  .  " + tAttributeName + ".dll" ) ;
								...
								if ( sGenericRelationClass != 0 ) {
									// Compose path from module path, module name, other relation class's class name and relation class name
										...
										// Get Module Path
											...
											CString tModuleFolder = tStructureFolderAttribute -> Value ( ) ;
										// Compose DLL path
											CString tLibraryPath = tModuleFolder + "\\" + ResolveInvalidFileNameCharacters ( tModuleName + "  .  " + tOtherRelationClassClassName + "  .  " + tRelationClassName + ".dll" ) ;
									// If DLL File Exists, load the module
										if ( FileExists ( tLibraryPath ) == TRUE ) {
											MethodLibraryHandle = LoadLibrary ( tLibraryPath ) ;	
														...
														if ( tOtherRelationClassPluralAttribute != 0 ) {
															...
															if ( tOtherRelationClassPlural == "False" && tTargetClassPlural == "True" ) {
																...
																if ( tRelationClassNameAttribute != 0 ) {
																	CString tRelationClassName = tRelationClassNameAttribute -> Value ( ) ;
																	if ( Item ( aIndex ) != 0 ) {
																		if ( Item ( aIndex ) -> RelatedItems ( ) -> Item ( tRelationClassName ) != 0 ) {
																			// When type safety would be on, the check above would not be necessary.
																			// When inheritance was fully implemented, assigning a 'derived' class wouldn't make this crash,
																			// when wanting to synchronize back to a base class member (that for now doesn't exist directly in the derived class).
								...
								if ( sGenericClassMethod != 0 ) {
									// Compose path from module path, module name, class name and method name
										...
										// Get Module Path
											...
											CString tModuleFolder = tStructureFolderAttribute -> Value ( ) ;
										// Compose DLL path
											CString tLibraryPath = tModuleFolder + "\\" + ResolveInvalidFileNameCharacters ( tModuleName + "  .  " + tClassName + "  .  " + tMethodName + ".dll" ) ;
									// If DLL File Exists, load the module
										if ( FileExists ( tLibraryPath ) == TRUE ) {
											MethodLibraryHandle = LoadLibrary ( tLibraryPath ) ;	
